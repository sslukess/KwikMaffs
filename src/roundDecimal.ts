...
const handleTenOnRound = (digitArray: string[], d: number, p: number, digitsAdded: number) => {
    ...
    devLog('handling ten on round');
    devLog('digit array: ', digitArray);
    devLog('control number: ', d);
    ...
    devLog('is long enough: ', isLongEnough);
    ...
    if (isLongEnough) { // we only do this for arrays with 2 or more digits
        if (d === 10) {
            ...
            devLog('new digit before last: ', newDigitBeforeLast);
            ...
            devLog('something went wrong!'); // this should never happen
        }
...
const roundArray = (digitArray: string[], p: number, digitsAdded: number) => {
    ...
    devLog('rounding this array: ', digitArray);
    ...
    devLog('control number: ', controlNumber);
    ...
    if (controlNumber >= 5) {
        ...
        devLog('added 1 to the digit before the last digit');
    } else if (controlNumber < 5) { // do nothing
        devLog('did not add 1 to the digit before the last digit');
    }
...
const roundDecimal = (number: number, p: number) => {
    ...
    devLog(`The number ${numberString} returns ${reDecimalNumberRounded} when rounded to ${p} decimal places.`)
